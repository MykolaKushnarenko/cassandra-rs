...
impl<'a, T: Write> ProtocolWriter<T> {
    ...
    pub fn send_request(&mut self, request: &Request) -> Result<(), std::io::Error> {
        ...
        let frame = Frame::new(Version::Request, None, None, Opcode::Query, request_bytes.len() as u32, request_bytes);
    ...
    pub fn send_response(&mut self, request: &Response) -> Result<(), std::io::Error> {
        ...
        let frame = Frame::new(Version::Response, None, None, Opcode::Query, request_bytes.len() as u32, request_bytes);
        let frame_bytes = frame.encode()?;
        self.writer.write_all(&frame_bytes)?;
        self.writer.flush()?;
